...
class Predictor:
    ...
    def predict_for_drug(self, drug_name: str, top_k: int = 20):
        ...
        with torch.no_grad():
            logits = self.classifier(drug_emb, disease_emb).numpy()

        # Per-query normalization: use the FULL distribution of logits across
        # all diseases for THIS drug as the reference range. The drug's own
        # best disease becomes the high end of the scale, the worst becomes 0.
        # This guarantees visible spread regardless of how saturated the
        # sigmoid is, while preserving the model's true ranking.
        all_min = float(logits.min())
        all_max = float(logits.max())
        denom = (all_max - all_min) + 1e-9
        ...
        for rank_pos, i in enumerate(top_idx):
            logit_val = float(logits[i])
            normalized = (logit_val - all_min) / denom
            ...
            results.append({
                ...
                "raw_score":  round(float(sigmoid_val), 4),
